...
(function(window){
    ...
    if (typeof window.UI === 'undefined') {
        window.UI = {
            ...
            updateAllViews: async function(){ return true; },
            showNotification: function(msg, type){
                console.log('UI.showNotification:', type, msg);
            },
            updateHeroSection: function(){},
            updateRecentMatches: function(){},
        };
    }
    ...
    // Expose showLoginModal expected by HTML
    window.showLoginModal = window.showLoginModal || function(){
        // Prefer UI.showLoginModal if exists
        if (typeof window.UI.showLoginModal === 'function') return window.UI.showLoginModal();
        // Otherwise, show element with id 'loginModal'
        openModal('loginModal');
        const input = document.getElementById('usernameInput') || document.getElementById('emailInput');
        if (input) input.focus();
    };

    // Expose closeLoginModal
    window.closeLoginModal = window.closeLoginModal || function(){
        closeModal('loginModal');
    };

    // Wire default login button behavior if present
    try {
        const btn = document.getElementById('btnLogin');
        if (btn && (!btn.onclick || btn.onclick.toString().includes('showLoginModal'))) {
            btn.addEventListener('click', function(e){
                e.preventDefault();
                if (RankedData && RankedData.currentUser) {
                    // logout
                    RankedData.logout().then(() => {
                        if (typeof window.updateUserDisplay === 'function') window.updateUserDisplay();
                        else if (document.getElementById('btnLogin')) document.getElementById('btnLogin').textContent = 'LOGIN';
                    }).catch(()=>{});
                } else {
